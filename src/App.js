...
import { decryptPayload, encryptPayload, generatePasteKey } from './utils/pasteHelper'
import './App.css';
...
class App extends Component {
  ...
  getData = async (hash) => {
    try {
      ...
      this.updateText(decryptedData.javascript || defaults.javascript.value, 'javascript')
  ...
  distributePanelsEvenly() {
    ...
    const visiblePanels = codeEditors.filter(panel => this.state.panels[panel].visible)
  ...
  renderPanels() {
    ...
    codeEditors.forEach(language => {
      if (this.state.panels[language].visible) panels.push(this.renderEditor(language))
    })
    return panels
  }

  onUpdatePanel = updatedWidths => {
    const updatedPanels = codeEditors.filter(p => this.state.panels[p].visible).reduce((acc, cur, i) => {
      acc[cur] = {
        visible: true,
